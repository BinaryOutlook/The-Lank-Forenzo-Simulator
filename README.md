...
- an accessible Options page with locally persisted wallpaper presets, ambient music, interaction sound cues, UI density, animation, and graphical-effect settings
...
Options are available from the app header. The current settings implementation is local-first and preset-based: wallpaper selection changes the shell background immediately, music can be toggled with a restrained Web Audio ambience, interaction cues follow the music volume and sound-effects toggle, and visual effects can be reduced or disabled for lower-end devices.
