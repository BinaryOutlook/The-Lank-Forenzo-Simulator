...
| FR-0022 | Overhauled sub-tab UI experience | [Brief](issue-briefs/0022-overhauled-sub-tab-ui-experience.md) | UI architecture / responsive UX | Lets each gameplay panel focus on its core job instead of forcing dense text and state into one cluttered adaptive surface. | L | High | High | Medium | Current run-screen panel inventory, mobile and desktop navigation model, route and tab ownership decision, text-density direction | Orange | P1 | Needs Architecture Review | None | Unassigned | Add as a concern after repeated density pressure in the current run UI. Needs architecture review before implementation because it changes navigation and panel ownership. |
| FR-0023 | Full and minimal text display modes | [Brief](issue-briefs/0023-text-display-density-modes.md) | Settings / accessibility / responsive UX | Gives players a full explanatory mode and a minimal phone-friendly mode so the game remains usable across aspect ratios and device sizes. | L | High | High | Medium | Settings model, UI copy inventory, mobile rendering audit, content summarization strategy, FR-0022 panel model decision | Orange | P1 | Needs Architecture Review | None | Unassigned | Add as a concern because current full-text presentation strains phone compatibility. Needs design review to decide whether minimal text is authored, generated, or component-level summarized. |
